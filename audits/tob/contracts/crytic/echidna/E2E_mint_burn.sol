...
import '../../../../../contracts/PrimeaV3Pool.sol';
...
contract E2E_mint_burn {
    ...
    constructor() public {
        ...
        primea = new SetupPrimea(token0, token1);

        minter = new PrimeaMinter(token0, token1);
        swapper = new PrimeaSwapper(token0, token1);
    ...
    function check_mint_invariants(
        int24 _tickLower,
        int24 _tickUpper,
        PrimeaMinter.MinterStats memory bfre,
        PrimeaMinter.MinterStats memory aftr
    ) internal {
        (, int24 currentTick, , , , , ) = pool.slot0();
    ...
    function _init(uint128 _seed) internal {
        ...
        // deploy the pool
        //
        primea.createPool(poolParams.fee, poolParams.startPrice);
        pool = primea.pool();
    ...
    function test_mint(uint128 _amount) public {
        ...
        (PrimeaMinter.MinterStats memory bfre, PrimeaMinter.MinterStats memory aftr) =
    ...
    function test_burn_partial(uint128 _amount) public {
        ...
        try minter.doBurn(pos.tickLower, pos.tickUpper, burnAmount) returns (
            PrimeaMinter.MinterStats memory bfre_burn,
            PrimeaMinter.MinterStats memory aftr_burn
        ) {
            bfre = bfre_burn;
            aftr = aftr_burn;
        } catch {
